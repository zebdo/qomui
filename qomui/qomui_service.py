...
class QomuiDbus(dbus.service.Object):
    ...
    version = "None"
    thread_list = []
    # https://stackoverflow.com/a/30711137
    ...
    def __init__(self):
        ...
        self.filehandler.setFormatter(logging.Formatter(
            '%(asctime)s - %(levelname)s - %(message)s'))
    ...
    def check_version(self):
        try:
            ...
            self.logger.warning(
                "{}/VERSION does not exist".format(config.ROOTDIR))
    ...
    def connect_to_server(self, ovpn_dict):
        ...
        setattr(self, "{}_dict".format(name), tunnel.TunnelThread(
            ovpn_dict, self.hop_dict, config.settings, self.interface))
    ...
    def disconnect(self, env):
        ...
        if env == "main":
            ...
            if self.wg_connect == 1:
                try:
                    wg_down = Popen(
                        ["wg-quick", "down", "{}/wg_qomui.conf".format(config.ROOTDIR)], stdout=PIPE, stderr=STDOUT)
                ...
                tunnel.exe_custom_scripts(
                    "down", self.wg_provider, config.settings)
                self.wg_connect = 0
    ...
    def kill_pid(self, i):
        if psutil.pid_exists(i[0]):
            ...
            try:
                self.logger.debug(
                    "OS: process {} killed - {}".format(i[0], i[1]))
                Popen(['kill', '{}'.format(i[0])])

            except CalledProcessError:
                self.logger.debug(
                    "OS: process {} does not exist anymore".format(i))
    ...
    def restore_default_dns(self):
        ...
        if resolver_check == 0:
            ...
            try:
                shutil.copyfile("/etc/resolv.conf.qomui.bak",
                                "/etc/resolv.conf")
                self.logger.debug("Restored backup of /etc/resolv.conf")

            except FileNotFoundError:
                self.logger.warning(
                    "Default DNS settings not restored. Could not find backup of /etc/resolv.conf")
    ...
    def import_thread(self, credentials):
        ...
        try:
            if credentials["credentials"] == "unknown":
                ...
                try:
                    auth_file = "{}/{}/{}-auth.txt".format(
                        config.ROOTDIR, provider, provider)
                    ...
                    self.logger.error(
                        "Could not find {} - Aborting update".format(auth_file))
    ...
    def start_import_thread(self, provider, credentials):
        setattr(self, "import_{}".format(provider),
                update.AddServers(credentials))
        getattr(self, "import_{}".format(provider)
                ).log.connect(self.log_thread)
        getattr(self, "import_{}".format(provider)
                ).finished.connect(self.downloaded)
        getattr(self, "import_{}".format(provider)
                ).failed.connect(self.imported)
        getattr(self, "import_{}".format(provider)
                ).started.connect(self.progress_bar)
    ...
    def downloaded(self, content):
        ...
        dns_manager.dns_request_exception(
            "-D", config.settings["alt_dns1"], config.settings["alt_dns2"], "53")

        if provider in config.SUPPORTED_PROVIDERS:
            with open('{}/config.json'.format(config.ROOTDIR), 'w') as save_config:
                config.settings["{}_last".format(
                    provider)] = str(datetime.utcnow())
                if provider == "Airvpn":
    ...
    def delete_provider(self, provider):
        ...
        if os.path.exists(path):
            ...
            try:
                os.remove(
                    "{}/certs/{}-auth.txt".format(config.ROOTDIR, provider))
            except FileNotFoundError:
                pass
    ...
    def update_thread(self):
        ...
        try:
            if self.packetmanager == "DEB":
                ...
                deb_url = "{}releases/download/v{}/{}".format(
                    base_url, self.version[1:], deb_pack)
                ...
                upgrade_cmd = ["dpkg", "-i",
                               "{}/{}".format(config.ROOTDIR, deb_pack)]

            elif self.packetmanager == "RPM":
                rpm_pack = "qomui-{}-1.x86_64.rpm".format(self.version[1:])
                rpm_url = "{}releases/download/v{}/{}".format(
                    base_url, self.version[1:], rpm_pack)
                ...
                upgrade_cmd = ["rpm", "-i",
                               "{}/{}".format(config.ROOTDIR, rpm_pack)]
            ...
            with open("{}/VERSION".format(config.ROOTDIR), "w") as vfile:
                if self.packetmanager != "None":
                    vfile.write("{}\n{}".format(
                        self.version[1:], self.packetmanager))
                else:
